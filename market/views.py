...
from .services import check_marketshare_criteria as calculate_uread_criteria
...
class MarketShareReportView(View):
    def get(self, request):
        ...
        # Get raw queryset
        market_data_qs = MarketShare.objects.filter(month=selected_month).select_related('ebook')

        # 1. หาผลรวมของ Uread ทั้งหมดในเดือนนั้นเพื่อใช้เป็นฐานในการคิด % ส่วนแบ่งตลาด
        total_monthly_uread = sum(item.uread for item in market_data_qs)

        # แปลง QuerySet เป็น List เพื่อให้เพิ่ม attributes ใหม่ๆ เข้าไปได้
        market_data = list(market_data_qs)

        for item in market_data:
            # --- คำนวณส่วนแบ่งตลาดรวม (Market Share) ---
            if total_monthly_uread > 0:
                item.market_share = (item.uread / total_monthly_uread) * 100
            else:
                item.market_share = 0.0

            # --- คำนวณ % คนอ่านผ่านเกณฑ์ (Pass Rate) ---
            # คิวรีหาจำนวนคนที่เข้ามาอ่านหนังสือเล่มนี้ (นับแบบไม่ซ้ำคน)
            total_readers = LogRead.objects.filter(ebook=item.ebook).values('user_id').distinct().count()

            if total_readers > 0:
                item.pass_rate = (item.uread / total_readers) * 100
            else:
                item.pass_rate = 0.0
        ...
        context = {
            ...
            'market_data': market_data  # ส่ง List ที่มี .market_share และ .pass_rate แนบไปด้วยแล้ว
        }
